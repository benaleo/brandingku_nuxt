  ...
  <section class="py-16 md:py-24 md:max-w-3/4 mx-auto">
    <div class="container px-4 mx-auto">
      ...
      <div v-else class="flex justify-center items-center gap-4 flex-wrap">
        <Card v-for="category in featuredCategories" :key="category.id" class="hover:shadow-md transition-shadow py-0 w-[120px]">
          <NuxtLink :to="`/category/${category.slug}`">
            <CardContent class="p-4 flex flex-col items-center">
              <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                <nuxt-img :src="`${STORAGE_URL}${category.image}`" width="100" height="100"/>
              </div>
